...
**CORREÇÃO PÓS-ENTREGA (11/07/2026): Flavio testou no iPhone e o alarme de um remédio recém-cadastrado não tocou.** Causa raiz encontrada: a proteção "não cancela alarme a menos de 2 min de tocar" bloqueava a reconciliação inteira mesmo quando o alarme NUNCA tinha sido agendado antes (caso do Flavio: horário de teste a poucos minutos do cadastro) — travava o 1º agendamento para sempre, sem nenhum aviso na tela. Corrigido com rastreamento de quais alarmes já estão de fato agendados (`lastScheduledKeys`); a proteção agora só vale para o que já existe. 2 rodadas de teste + revisor-seguranca + revisor-codigo aprovaram (119/119 testes). Limitação residual documentada em DESAFIOS.md item 15 (não é bug: alarme novo pode ficar adiado, não perdido, se coincidir com outro já iminente).
